...
module.exports = async (deployer, network) => {
    ...
    const rootChainManager = await BscRootChainManager.at(contractAddresses.root.BscRootChainManagerProxy);
    const childChainManager = await ChildChainManager.at(contractAddresses.child.BscChildChainManagerProxy);

    const BnbPredicateInstance = await BnbPredicate.at(contractAddresses.root.BnbPredicate)
    const ERC20PredicateInstance = await ERC20Predicate.at(contractAddresses.root.BscERC20Predicate)
    const ERC721PredicateInstance = await ERC721Predicate.at(contractAddresses.root.BscERC721Predicate)
